...
partial model PartialHVAC_filt
  ...
  parameter Real ratOAFlo_A(unit="m3/(s.m2)")=0.3e-3
  ...
  parameter Real ratP_A = 5e-2
    "Occupant density";
  parameter Real effZ(unit="1")=0.8
    "Zone air distribution effectiveness (limiting value)";
  parameter Real divP(unit="1")=0.7
  ...
  parameter Real effVen(unit="1")=if divP < 0.6 then 0.88*divP + 0.22 else 0.75
  ...
  parameter Modelica.Units.SI.Temperature TCooOff=303.15
    ...
    annotation (Dialog(group="Room temperature setpoints"));
  parameter Modelica.Units.SI.PressureDifference dpBuiStaSet=12
  ...
  parameter Modelica.Units.SI.MassFraction wCooAirMix_nominal = 0.017
    ...
    annotation (Dialog(group="Air handler unit nominal temperatures and humidity"));
   parameter Modelica.Units.SI.Temperature TCooWatInl_nominal(displayUnit=
        "degC")=279.15
